...
function Home() {
  const calculateTimeLeft = () => {
    const difference = +new Date("2025-04-05T00:00:00") - +new Date();
    let timeLeft = {};
  ...
  return (
    <div className="w-full min-h-screen bg-black text-white flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full flex flex-col items-center justify-center text-center py-20">
        ...
        >
          <h2 className="text-purple-400 text-xl font-bold">IEEE WIE PRESENTS</h2>
          ELYSIUM
      ...
      <div className="w-full flex flex-col items-center justify-center py-20">
        {Object.keys(timeLeft).length > 0 ? (
          <motion.div 
            ...
            transition={{ duration: 1 }}
          >
            <h2 className="text-2xl font-bold text-purple-400">Event Starts In:</h2>
            <motion.p 
          ...
          <h2 className="text-2xl font-bold text-red-500">The event has started!</h2>
        )}
      </div>

      
      <motion.img 
        src="photos\\robo2.webp" 
        alt="Futuristic AI Head"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.5 }}
        className="absolute right-10 top-30 h-[200px] object-contain"
      />
    </div>
  );
}
